...
const LernendeAddForm = () => {
    ...
    useEffect(() => {
      fetch('https://aurelio.undefiniert.ch/tbl_lernende/id_lernende/' + id).then((response) => response.json())
    ...
    if(id){
      setFormData({
        ...
        email_privat: selectedKurs.email_privat,
...
`\{
    ...
    "plz" : ${formData.plz},
    "ort" : \"${formData.ort}\",
    "nr_land" : \"${selectedDozent.value}\",
    "geschlecht" : \"${formData.geschlecht}\",
    "telefon" : \"${formData.telefon}\",
    "handy" : \"${formData.handy}\",
    "email" : \"${formData.email}\",
    "email_privat" : \"${formData.email_privat}\",
    "birthdate" : \"${formatDate(formData.brithDate)}\",
\}`,
    })
      .then((response) => response.json())
      .then((data) => {
  ...
  return (
    <div className="container mt-5">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          <label htmlFor="email_privat">Email Private:</label>
          <input
            type="text"
            className="form-control"
            id="email"
